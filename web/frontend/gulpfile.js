...
gulp.task('sass', function () {
  return gulp.src('./scss/**/*.scss')
      ...
      .pipe(gulp.dest('../docroot/stylesheets'))      // 1. output for webelopment with sourcemaps normal css
      .pipe(cleanCSS())                           // gulp css minify
      .pipe(rename({suffix: '.min'}))             // gulp.rename (added .min)
      .pipe(gulp.dest('../docroot/stylesheets'))      // 2. output just minimized
      .pipe(gzip({ gzipOptions: { level: 9 } }))  // compress with highest compression
      .pipe(gulp.dest('../docroot/stylesheets'));     // 3. output gzipped
});
